                        ...
                        <div id="dynamic-fields" style="{{ old('role') == 'student' ? 'display:block;' : 'display:none;' }}">
                            ...
                            <div id="student-fields" style="{{ old('role') == 'student' ? 'display:block;' : 'display:none;' }}">
                                <div class="form-floating-custom mb-1" id="wrap-student_number">
                                    <input type="text" name="student_number" id="student_number" placeholder=" " maxlength="50" value="{{ old('student_number') }}">
                                    <label for="student_number">Student ID (Optional — Auto-generated if empty)</label>
                                </div>
        ...
        function validateStep1Full(showInlineErrors = true) {
            ...
            if (!roleChecked) {
                ...
                if (roleChecked.value === 'student') {
                    ...
                    // Student ID (Optional — system auto-assigns if blank)
                    const sNumVal = sNum ? sNum.value.trim() : '';
                    if (!sNumVal) {
                        if (showInlineErrors) setFieldFeedback('student_number', true, 'Will be auto-generated by the system.');
                    } else if (!/^[a-zA-Z0-9-]{4,50}$/.test(sNumVal)) {
                        if (showInlineErrors) setFieldFeedback('student_number', false, 'Student ID format is invalid.');
                        isValid = false;
                        if (!firstErrorMsg) firstErrorMsg = 'Student ID format is invalid.';
                    } else {
