...
class AuditAttendanceQrScan
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        try {
            ...
            $accepted = $response->isSuccessful() && $inspection['status'] === 'valid' && !$already;
            $reason = $accepted ? null : ($body['error_type'] ?? ($already ? 'duplicate' : $inspection['status']));
            ...
            $activity->log($accepted ? 'qr_scanned' : 'scan_rejected');
