...
class QrAttendanceController extends Controller
{
    ...
    // ─────────────────────────────────────────
    // Teacher: Update session classroom location & geofence
    // ─────────────────────────────────────────
    public function updateSessionLocation(Request $request)
    {
        ...
        return response()->json([
            'success'       => true,
            'message'       => 'Classroom location synced successfully.',
            'classroom_lat' => (float) $session->classroom_lat,
            'classroom_lng' => (float) $session->classroom_lng,
            'radius_meters' => (int) $session->getAllowedRadius(),
        ]);
    }
    // ─────────────────────────────────────────
    ...
    public function verifyPresence(Request $request)
    {
        ...
        if ($radius <= 0) {
            $attendance->update([
                'last_location_check_at'    => $now,
                'last_latitude'             => $request->filled('latitude') ? (float) $request->latitude : null,
                'last_longitude'            => $request->filled('longitude') ? (float) $request->longitude : null,
                'last_accuracy'             => $request->filled('accuracy') ? (float) $request->accuracy : null,
            ...
            return response()->json([
                ...
                'distance'          => 0,
                'radius'            => 0,
                'message'           => 'Presence verified (Geofence disabled).'
            ]);
        }
    ...
    public function completeVerification(Request $request, WebauthnService $webauthn)
    {
        ...
        if ($radiusMeters > 0) {
            ...
            // A 50m classroom boundary cannot be decided from a wider GPS uncertainty.
            if ($studentAccuracy > 50) {
                return response()->json([
                    'success'    => false,
                    'error_type' => 'unreliable_gps',
                    'accuracy'   => round($studentAccuracy),
                    'radius'     => $radiusMeters,
                    'message'    => 'GPS signal accuracy is too low (±' . round($studentAccuracy) . 'm) to verify your classroom location. Please move near a window, enable High Accuracy GPS, and try again.'
                ], 422);
            }
            ...
            if (!$coords['disabled']) {
                ...
                if (config('app.debug')) {
                    Log::debug('QR geofence distance check', [
                        'attendance_session_id' => $session->id,
                        'teacher_latitude' => $schoolLat,
                        'teacher_longitude' => $schoolLng,
                        'student_latitude' => $studentLat,
                        'student_longitude' => $studentLng,
                        'student_accuracy_meters' => $studentAccuracy,
                        'raw_distance_meters' => $distance,
                        'final_distance_meters' => $distance,
                ...
                if ($distance > $radiusMeters) {
                    ...
                    return response()->json([
                        ...
                        'distance'           => round($distance),
                        'effective_distance' => round($distance),
                        'radius'             => $radiusMeters,
        ...
        if ($radiusMeters > 0) {
            if ($session->classroom_lat === null || $session->classroom_lng === null) {
                return response()->json([
                    'success' => false,
                    'error_type' => 'teacher_location_unavailable',
                    'message' => 'The teacher\'s laptop location is not available for this session. Please ask your instructor to update it.'
                ], 422);
            }
            if (!$hasCoordinates) {
                return response()->json([
                    ...
                    'message' => 'A fresh student device GPS location is required. Enable location and retry.'
                ], 422);
            }
            if ($hasCoordinates) {
                [$studentLat, $studentLng] = self::normalizeCoordinates((float) $request->latitude, (float) $request->longitude);
                ...
                if ($accuracy > 50) {
                    return response()->json([
                        ...
                        'radius'     => $radiusMeters,
                        'message'    => 'GPS signal accuracy is too low (±' . round($accuracy) . 'm) to verify your classroom location. Please move near a window, enable High Accuracy GPS, and try again.'
                    ], 422);
                }
                ...
                if (!$coords['disabled']) {
                    ...
                    if (config('app.debug')) {
                        Log::debug('QR scan geofence distance check', [
                            'attendance_session_id' => $session->id,
                            'teacher_latitude' => $schoolLat,
                            'teacher_longitude' => $schoolLng,
                            'student_latitude' => $studentLat,
                            'student_longitude' => $studentLng,
                            'student_accuracy_meters' => $accuracy,
                            'raw_distance_meters' => $distance,
                            'final_distance_meters' => $distance,
                            'allowed_radius_meters' => $radiusMeters,
                        ]);
